...
const LoginPage: React.FC = () => {
  ...
  const { login, isLoading, currentUser } = useAppContext();
  const navigate = useNavigate();
  const location = useLocation();
  const from = location.state?.from?.pathname || "/app/dashboard";

  React.useEffect(() => {
    if(currentUser && !('isAdmin' in currentUser && currentUser.isAdmin)){ // Redirect if already logged in as user
...
import { Landmark } from 'lucide-react'; // Import Landmark
