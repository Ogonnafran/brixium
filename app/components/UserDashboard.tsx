...
const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode; currency?: string }> = ({ title, value, icon, currency }) => (
  <div className="bg-brixium-bg-light p-6 rounded-xl shadow-lg transform hover:scale-105 transition-transform duration-300">
    ...
    <p className="text-3xl font-bold text-white">
      ...
      {currency && <span className="text-xl ml-1 text-brixium-gray">{currency}</span>}
    </p>
  </div>
);
...
const TransactionItem: React.FC<{ tx: Transaction }> = ({ tx }) => {
  ...
  const isCredit = tx.type === TransactionType.DEPOSIT || (tx.type === TransactionType.TRANSFER && tx.toUserId === (useAppContext().currentUser as User)?.id);
  const isDebit = tx.type === TransactionType.WITHDRAWAL || (tx.type === TransactionType.TRANSFER && tx.fromUserId === (useAppContext().currentUser as User)?.id) || tx.type === TransactionType.FEE;
  ...
  return (
    <li className="py-4 px-2 flex justify-between items-center border-b border-brixium-gray-dark/30 hover:bg-brixium-purple/10 transition-colors">
      <div className="flex items-center">
        <div className={`mr-3 p-2 rounded-full ${isCredit ? 'bg-green-500/20 text-green-400' : isDebit ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
          {isCredit ? <TrendingUp size={20} /> : isDebit ? <TrendingDown size={20} /> : <Clock size={20} />}
        </div>
        <div>
          <p className="text-sm font-medium text-white">{tx.type} - {description}</p>
          <p className="text-xs text-brixium-gray">{new Date(tx.date).toLocaleString()}</p>
        </div>
      </div>
      <p className={`text-sm font-semibold ${isCredit ? 'text-green-400' : isDebit ? 'text-red-400' : 'text-blue-400'}`}>
        {isCredit ? '+' : isDebit ? '-' : ''}
