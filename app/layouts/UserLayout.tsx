...
const UserLayout: React.FC = () => {
  const { currentUser, logout, getUserNotifications, markNotificationAsRead, appSettings } = useAppContext();
  const navigate = useNavigate();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [notificationsOpen, setNotificationsOpen] = useState(false);

  if (!currentUser || 'hashedPassword' in currentUser && currentUser.email === 'brixiumglobalbank@gmail.com') { // Basic check for admin
    // This case should ideally be handled by ProtectedRoute, but as a safeguard
    navigate('/app/login');
    return null;
  }
  
  const user = currentUser as import('../../types').User; // Type assertion
  ...
  const toggleNotifications = () => {
    ...
    if(!notificationsOpen && unreadCount > 0){ // Mark as read when opening
