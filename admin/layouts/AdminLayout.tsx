...
const AdminLayout: React.FC = () => {
  const { currentUser, logout, getAdminNotifications, markNotificationAsRead, appSettings } = useAppContext();
  const navigate = useNavigate();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [notificationsOpen, setNotificationsOpen] = useState(false);

  if (!currentUser || !('email' in currentUser && currentUser.email === 'brixiumglobalbank@gmail.com')) {
     navigate('/admin/login');
     return null;
  }
  
  const adminUser = currentUser as import('../../types').AdminUser;
