...
import { User, TransactionStatus, KYCStatus, Currency } from '../../types';
import Icon from '../../components/common/Icon';
import { Users, DollarSign, FileText, Send, AlertTriangle } from 'lucide-react';

interface StatCardProps {
  title: string;
  value: string | number;
  iconName: import('lucide-react').LucideIcon;
  colorClass: string; // e.g., text-green-400
  bgColorClass: string; // e.g., bg-green-500/10
  unit?: string;
}
...
const AdminDashboardSummary: React.FC = () => {
  ...
  // Calculate platform balance summary (simplified: sum all USD equivalent balances)
  // This is a very rough estimate as exchange rates vary.
  // For a real app, this would be a complex calculation or stored value.
  const platformBalanceUSD = users.reduce((sum, user) => {
    let balanceInUSD = user.balance;
    if (user.currency !== Currency.USD) {
      // Crude conversion - a real system would use daily rates or have balances in a base currency
  ...
  return (
    <div className="space-y-8 animate-slide-in-up">
      ...
      {/* TODO: Add more charts or summaries here, e.g., recent admin notifications, transaction volume chart */}
       <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-brixium-bg-light p-6 rounded-xl shadow-lg">
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li className="flex justify-between"><span>Network Fee:</span> <span className="font-semibold text-white">{appSettings.defaultNetworkFee} ({appSettings.networkFeeType})</span></li>
          </ul>
        </div>
        <div className="bg-brixium-bg-light p-6 rounded-xl shadow-lg">
            <h3 className="text-xl font-semibold text-brixium-purple-light mb-4">System Health</h3>
            {/* Mock system health. In a real app, these would come from monitoring. */}
            <div className="space-y-3">
                <div className="flex items-center"><div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div> API Services: Operational</div>
                <div className="flex items-center"><div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div> Database Connectivity: Nominal</div>
                <div className={`flex items-center ${appSettings.maintenanceMode ? 'text-yellow-400' : 'text-green-400'}`}>
                    <div className={`w-3 h-3 ${appSettings.maintenanceMode ? 'bg-yellow-500' : 'bg-green-500'} rounded-full mr-2`}></div> 
...
// MOCK_EXCHANGE_RATES needs to be accessible here or passed via props/context if not global
// For simplicity, assuming it's imported if constants.ts is available globally (not ideal)
// Better: const { MOCK_EXCHANGE_RATES } = useAppContext() if it were part of context.
// Or just rely on constants.ts import.
