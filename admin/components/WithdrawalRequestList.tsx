...
import Input from '../../components/common/Input'; // For search
import { Send, CheckCircle, XCircle, Eye, Filter, Search } from 'lucide-react';
...
const WithdrawalRequestList: React.FC = () => {
  const { withdrawalRequests, users, updateWithdrawalStatus, isLoading, findUserById } = useAppContext();
  const [filterStatus, setFilterStatus] = useState<TransactionStatus | 'all'>('all');
  const [searchTerm, setSearchTerm] = useState(''); // Search by User Name/ID or Wallet Address
  ...
  const handleAction = (requestId: string, status: TransactionStatus.COMPLETED | TransactionStatus.REJECTED) => {
    // Assuming admin ID is 'admin-001' for mock
  ...
  return (
    <div className="bg-brixium-bg-light p-4 sm:p-6 rounded-xl shadow-xl animate-slide-in-up">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
        ...
        <div className="flex items-center gap-2">
            ...
            <select 
                ...
                className="bg-brixium-bg border border-brixium-gray-dark text-brixium-gray-light text-sm rounded-lg focus:ring-brixium-purple focus:border-brixium-purple p-2.5"
            >
      ...
      {filteredRequests.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRequests.map(req => {
            const user = req.user;
            return (
              <div key={req.id} className="bg-brixium-bg p-5 rounded-lg shadow-md border border-brixium-gray-dark/50 transform hover:shadow-brixium-purple/20 transition-shadow">
                <div className="flex justify-between items-start mb-2">
                    <div>
                        ...
                        <p className="text-sm text-brixium-gray-light">{user?.email}</p>
                    </div>
                ...
                <div className="mt-4 flex space-x-2 justify-end">
                  <Button variant="ghost" size="sm" onClick={() => viewRequestDetails(req)} className="!px-3 !py-1.5">
                    <Eye size={16} className="mr-1.5" /> Details
                  </Button>
                  {req.status === TransactionStatus.PENDING && (
                    <>
                      <Button variant="primary" size="sm" onClick={() => handleAction(req.id, TransactionStatus.COMPLETED)} isLoading={isLoading} className="!px-3 !py-1.5 bg-green-500 hover:bg-green-600">
                        <CheckCircle size={16} className="mr-1.5" /> Approve
                      </Button>
                      <Button variant="danger" size="sm" onClick={() => handleAction(req.id, TransactionStatus.REJECTED)} isLoading={isLoading} className="!px-3 !py-1.5">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Withdrawal Request: ${selectedRequest?.id}`}>
        {selectedRequest && (
            <div className="text-brixium-gray-light space-y-2">
                ...
                {selectedRequest.status === TransactionStatus.PENDING && (
                    <div className="mt-6 flex justify-end space-x-3">
                        <Button variant="primary" onClick={() => handleAction(selectedRequest.id, TransactionStatus.COMPLETED)} isLoading={isLoading} className="bg-green-500 hover:bg-green-600">
                            Approve Withdrawal
                        </Button>
                        <Button variant="danger" onClick={() => handleAction(selectedRequest.id, TransactionStatus.REJECTED)} isLoading={isLoading}>
                            Reject Withdrawal
                        </Button>
                    </div>
                )}
