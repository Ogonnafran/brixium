...
import { ShieldCheck, ShieldAlert, CheckCircle, XCircle, Eye, Filter, Search } from 'lucide-react';
...
const KycRequestList: React.FC = () => {
  const { kycRequests, users, updateKYCStatus, isLoading, findUserById } = useAppContext();
  const [filterStatus, setFilterStatus] = useState<KYCStatus | 'all'>('all');
  const [searchTerm, setSearchTerm] = useState(''); // Search by User Name or ID
  ...
  return (
    <div className="bg-brixium-bg-light p-4 sm:p-6 rounded-xl shadow-xl animate-slide-in-up">
              ...
              <div key={req.id} className="bg-brixium-bg p-5 rounded-lg shadow-md border border-brixium-gray-dark/50 transform hover:shadow-brixium-purple/20 transition-shadow">
                ...
                <div className="mt-4 flex space-x-2 justify-end">
                  ...
                  {req.status === KYCStatus.PENDING && (
                    <>
                      <Button variant="primary" size="sm" onClick={() => handleAction(req.id, KYCStatus.APPROVED)} isLoading={isLoading} className="!px-3 !py-1.5 bg-green-500 hover:bg-green-600">
                        <CheckCircle size={16} className="mr-1.5" /> Approve
                      </Button>
                      <Button variant="danger" size="sm" onClick={() => handleAction(req.id, KYCStatus.REJECTED)} isLoading={isLoading} className="!px-3 !py-1.5">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`KYC Documents for ${selectedRequest?.user?.name || 'Request'}`}>
        {selectedRequest && (
            <div className="text-brixium-gray-light">
                        ...
                        {selectedRequest.documentUrls.map((url, index) => (
                            <li key={index} className="truncate">
                                <a href={url} target="_blank" rel="noopener noreferrer" className="text-brixium-purple-light hover:underline">
                                    Mock_Document_{index + 1}.pdf {/* In real app, use actual file name */}
                                </a>
                            </li>
                        ))}
                    </ul>
                ...
                {selectedRequest.status === KYCStatus.PENDING && (
                    <div className="mt-6 flex justify-end space-x-3">
                        <Button variant="primary" onClick={() => handleAction(selectedRequest.id, KYCStatus.APPROVED)} isLoading={isLoading} className="bg-green-500 hover:bg-green-600">
                            Approve
                        </Button>
                        <Button variant="danger" onClick={() => handleAction(selectedRequest.id, KYCStatus.REJECTED)} isLoading={isLoading}>
                            Reject
                        </Button>
                    </div>
                )}
