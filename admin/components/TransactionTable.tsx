...
import { ListFilter, Search, ArrowRightLeft, TrendingUp, TrendingDown, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
const TransactionTable: React.FC = () => {
  ...
  const sortedTransactions = useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        // Handle 'userName' separately as it's not a direct key of Transaction
        const valA = sortConfig.key === 'userName' ? a.userName : a[sortConfig.key as keyof Transaction];
        const valB = sortConfig.key === 'userName' ? b.userName : b[sortConfig.key as keyof Transaction];
  ...
  const handleWithdrawalAction = (txId: string, approve: boolean) => {
    // Assuming this transaction ID corresponds to a withdrawal request ID
    updateWithdrawalStatus(txId, approve ? TransactionStatus.COMPLETED : TransactionStatus.REJECTED, 'admin-001');
    // Potentially refresh transactions or rely on context update
  };
  ...
  return (
    <div className="bg-brixium-bg-light p-4 sm:p-6 rounded-xl shadow-xl animate-slide-in-up">
      ...
      <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title={`Transaction Details: ${selectedTransaction?.id}`}>
        {selectedTransaction && (
          <div className="space-y-3 text-sm text-brixium-gray-light">
            ...
            {selectedTransaction.toAddress && <p><strong>To Address:</strong> {selectedTransaction.toAddress}</p>}
...
// Import necessary icons if not already part of global Icon component imports
import { Repeat, DollarSign, CheckCircle, XCircle } from 'lucide-react';
